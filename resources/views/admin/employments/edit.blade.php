...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.employments.update", [$employment->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="brief">{{ trans('cruds.employment.fields.brief') }}</label>
                <textarea class="form-control {{ $errors->has('brief') ? 'is-invalid' : '' }}" name="brief" id="brief">{{ old('brief', $employment->brief) }}</textarea>
                @if($errors->has('brief'))
                    <div class="invalid-feedback">
                        {{ $errors->first('brief') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.employment.fields.brief_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="job_id">{{ trans('cruds.employment.fields.job') }}</label>
                <select class="form-control select2 {{ $errors->has('job') ? 'is-invalid' : '' }}" name="job_id" id="job_id" required>
                    @foreach($jobs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('job_id') ? old('job_id') : $employment->job->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('job'))
                    <div class="invalid-feedback">
