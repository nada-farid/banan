...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.newss.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="address">{{ trans('cruds.news.fields.address') }}</label>
            ...
            <div class="form-group">
                <label for="file">الملف المرفق</label>
                ...
                @endif
                <span class="help-block">يمكن رفع ملف PDF أو Word أو أي نوع ملف</span>
            </div>
            <div class="form-group">
...
<script>
    ...
    url: '{{ route('admin.newss.storeMedia') }}',
    maxFilesize: 10, // MB
    acceptedFiles: '.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.zip,.rar',
    maxFiles: 1,
    addRemoveLinks: true,
    headers: {
      'X-CSRF-TOKEN': "{{ csrf_token() }}"
...
@if(isset($news) && $news->file)
      var file = {!! json_encode($news->file) !!}
          this.options.addedfile.call(this, file)
      this.options.thumbnail.call(this, file, file.preview ?? file.preview_url)
      file.previewElement.classList.add('dz-complete')
